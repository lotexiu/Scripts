...
def search_files(type_file=None, path='.'):
    """**.. = previous directory**\n
    print(shocurasy.search_files('mid', '../MidiPlayer'))\n
    print(shocurasy.search_files('py', '../..'))\n
    print(shocurasy.search_files())"""
    import os
...
def show_pages(content_list, total_pages, num_pag='!p1', search=None):
    """x = shocurasy.search_files('', '../MidiPlayer')\n
    total_pags = shocurasy.pags(x, 12)\n
    choose = '!p2'\n
    shocurasy.show_pages(x, total_pags, choose, 'hanz')"""
    print('\nSongs:')
    range_page = 0
    while True:
        range_page += 1
